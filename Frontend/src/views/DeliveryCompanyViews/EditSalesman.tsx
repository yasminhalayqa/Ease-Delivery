...
export default function EditSalesman() {
    ...
    const { data } = useFetchData(`http://localhost:3000/users/salesman/${params.id}`) as any
    const { putData, loading } = usePutData(`http://localhost:3000/users/salesman/${params.id}`)
    ...
    useEffect(() => {
        if (data) {
            setName(data?.salesMan?.name || '');
            setAddress(data?.salesMan?.address || '');
            setEmail(data?.salesMan?.email || '');
            setPhoneNumber(data?.salesMan?.phone_number || '');
            // setPassword(data?.salesMan?.password || '');
            setCompnayCity(data?.salesMan?.city?.name || '');
            setArea(data?.salesMan?.area?.name || '');
        ...
        <MiniDrawer>
            <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '3px' }}>
                <Card sx={{ maxWidth: '850px' }}>
                    <CardHeader title=" تعديل بيانات مندوب التوصيل " sx={{
                    ...
                    <CardContent>
                        <Grid className="d-flex justify-content-center" container columnSpacing={{ xs: 1, sm: 2, md: 1 }} >
                            ...
                            <Input name="العنوان بالكامل" value={address} handler={(e: any) => setAddress(e.target.value)} error={isEmpty && city.trim() === ''} />
                        </Grid>
