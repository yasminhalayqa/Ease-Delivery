...
export default function SalesmanOrders() {
    ...
    const salesman_id = sessionStorage.getItem('id')

    const rows = orders?.orders?.filter((item: any) => item.salesman_id == salesman_id && item.status != "قيد الانتظار").map((order: any) => ({
        ...order,
        id: order.order_id,
