...
export default function StoreOrders() {
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Button
                    ...
                    variant="contained"
                    color="primary"
                    onClick={() => navigate(`/store/order_details_status/${params.row.id}`)
                    }
                >
                    عرض الحالة
                </Button>
    ...
    const rows = orders?.orders?.filter((item: any) => item.store_id == store_id).map((order: any) => ({
        ...order,
        id: order.order_id,
                ...
                <Grid item xs={12} md={12} className="mb-3 mt-1">
                    <Card>
                        <CardContent className="d-flex justify-content-between align-items-center">
                            ...
                            <Box sx={{ display: 'flex' }}>
                                <CustomButton text='شحنة جديدة' bg='#23af62' hoverBg='#2bb569' onClick={() => navigate('/store/create_order')} />
